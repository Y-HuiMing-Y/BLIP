...
def load_checkpoint(model, url_or_filename):
    ...
    state_dict = checkpoint['model']  # 获取checkpoint字典中键为model的值

    state_dict['visual_encoder.pos_embed'] = interpolate_pos_embed(state_dict['visual_encoder.pos_embed'],
                                                                   model.visual_encoder)  # 对旧模型的位置信息嵌入进行插值，以适应当前模型的结构
    if 'visual_encoder_m.pos_embed' in model.state_dict().keys():
        state_dict['visual_encoder_m.pos_embed'] = interpolate_pos_embed(state_dict['visual_encoder_m.pos_embed'],
                                                                         model.visual_encoder_m)
    # 检查参数的形状是否匹配。如果不匹配，则从 state_dict 中删除对应的参数
    for key in model.state_dict().keys():
        if key in state_dict.keys():
            if state_dict[key].shape != model.state_dict()[key].shape:
                del state_dict[key]

    msg = model.load_state_dict(state_dict, strict=False)  # 将加载的状态字典应用到模型中，并赋值给msg
    '''
    在 strict=False 的情况下，模型会尝试加载所有能够匹配的参数，但不会报错。
    这在一些情况下很有用，比如模型结构发生变化但只想加载共享部分参数，或者加载预训练模型但只想加载某些层的参数等。
    '''
